...
/*void Texture::texturesUpdate() {
	...
	if (Texture::toUpdate>0 && Texture::toUpdate<=Texture::buffer.size()) {// <editor-fold defaultstate="collapsed" desc="toUpdate">
		printf("toUpdate %i\n", Texture::toUpdate);
		uint updated = 0;
		for(int i=0; i<Texture::buffer.size(); i++) {
			...
			if (tex->img==NULL) {// <editor-fold defaultstate="collapsed" desc="tex->img==NULL">
				...
				if (tex->img->status!=Image::LOADED) continue;
				printf("\t//////\n");
			...
			//
			//
			if (updated>=Texture::toUpdate) break;//</editor-fold>
		}
	}// </editor-fold>
	if (Texture::toDelete>0 && Texture::toDelete<=Texture::buffer.size()) {// <editor-fold defaultstate="collapsed" desc="toDelete">
		printf("toDelete\n");
		uint deleted = 0;
		for(int i=0; i<Texture::buffer.size(); i++) {
			if (Texture::buffer[i]->event==EVENT::TO_DELETE) {
				delete Texture::buffer[i];
				Texture::buffer[i] = Texture::buffer[ Texture::buffer.size()-1 ];
				Texture::buffer.pop_back();
				deleted++;
				if (deleted>=Texture::toDelete) break; 
			}
		}
	}// </editor-fold>
}*/
